...
use Modules\CourseApi\app\Http\Requests\CourseStoreRequest;
use Modules\CourseApi\app\Models\CourseCategory;
use Modules\CourseApi\app\Models\CourseLanguage;
use Modules\CourseApi\app\Models\CourseLevel;
...
class CourseController extends Controller
{
    function index(Request $request): View
    {
        ...
        return view('courseapi::course.index', compact('courses', 'categories', 'instructors'));
    }
    ...
    function store(CourseStoreRequest $request)
    {
        ...
        $course->api_course_id = $request->api_course_id;
        ...
        return response()->json([
            ...
            'redirect' => route('admin.coursesapi.edit', ['id' => $course->id, 'step' => $request->next_step])
        ]);
    }
    ...
    function edit(Request $request)
    {
        if (!Session::get('course_create')) {
            return redirect(route('admin.coursesapi.create'));
        }
        ...
        switch (request('step')) {
            case '1':
                ...
                return view('courseapi::course.create', compact('course', 'editMode', 'instructors', 'courses'));
                break;
            case '2':
                $courseId = request('id');
                $categories = CourseCategory::where('status', 1)->get();
                ...
                return view('courseapi::course.more-information', compact(
                    'categories',
                    'courseId',
                    'course',
                    'levels',
            ...
            case '3':
                ...
                return view('courseapi::course.course-content', compact('chapters', 'grades'));
                break;
            case '4':
                $courseId = request('id');
                $course = Course::findOrFail($courseId);
                return view('courseapi::course.finish', compact('course'));
                break;
            default:
                break;
        }
        ...
        switch ($request->step) {
            case '2':
                ...
                return response()->json([
                    'status' => 'success',
                    'message' => __('Updated Successfully'),
                    'redirect' => route('admin.coursesapi.edit', ['id' => Session::get('course_create'), 'step' => $request->next_step])
                ]);
                break;
            case '3':
                return response()->json([
            ...
            case '4':
                ...
                return response()->json([
                    'status' => 'success',
                    'message' => __('Updated Successfully'),
                    'redirect' => $request->next_step == 4 ? route('admin.coursesapi.index') : route('admin.coursesapi.edit', ['id' => Session::get('course_create'), 'step' => $request->next_step])
                ]);
