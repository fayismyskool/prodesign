@if (Module::isEnabled('CourseApi') && Route::has('admin.courses.index'))
    @php
        $pendingCourseCount = \App\Models\Course::where('is_approved', 'pending')->count();
    @endphp
    <li
        class="nav-item dropdown {{ isRoute(['admin.coursesapi.*', 'admin.course-category.*', 'admin.course-filter.*', 'admin.course-language.*', 'admin.course-level.*', 'admin.course-grade.*', 'admin.course-review.*', 'admin.course-delete-request.*', 'admin.course-sub-category.*'], 'active') }}">
        ...
        <ul class="dropdown-menu">
            ...
            <li class="{{ isRoute('admin.course-grade.*', 'active') }}">
                <a class="nav-link" href="{{ route('admin.course-grade.index') }}">
                    {{ __('Grades') }}
                </a>
            </li>
